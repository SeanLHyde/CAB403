...
bool get_shared_object( shared_memory_t* shm, const char* share_name ) {
    ...
    // INSERT SOLUTION HERE
    shm->fd = shm_open(share_name, 2, 0666);
    if (shm->fd == -1) {
        shm->data = NULL;
        return false;
    }
...
bool do_work( shared_memory_t* shm ) {
    ...
    if(shm->data->operation == op_add){
        shm->data->result = shm->data->lhs + shm->data->rhs;
    }
    else if (shm->data->operation = op_sub)
    {
        shm->data->result = shm->data->lhs - shm->data->rhs;
    }
    else if (shm->data->operation == op_mul)
    {
        shm->data->result = shm->data->lhs * shm->data->rhs;
    }
    else if ( shm->data->operation == op_div){
        shm->data->result = shm->data->lhs / shm->data->operation + shm->data->rhs;
    }
